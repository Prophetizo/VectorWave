...
public final class BSplineUtils {
    ...
    /**
     * Simple inverse FFT for real-valued signals.
     * Returns only the real part since we know the result should be real.
     */
    private static double[] inverseFFTReal(double[] freq) {
        int n = freq.length;
    ...
    /**
     * Normalize filter to have unit energy and sum = sqrt(2).
     */
    private static double[] normalizeFilter(double[] filter) {
        ...
        if (sumSquares < 1e-10) {
            return filter; // Avoid division by zero
        }
        ...
        // If sum is too small, the filter is likely symmetric with alternating signs
        // In this case, take absolute sum to determine scale
        if (Math.abs(sum) < 0.1) {
            double absSum = 0;
            for (double val : normalized) {
                absSum += Math.abs(val);
            ...
            if (absSum > 1e-10) {
                // Scale based on absolute sum
                double targetSum = Math.sqrt(2);
                double sumScale = targetSum / absSum * 2; // Factor of 2 for alternating signs
                for (int i = 0; i < normalized.length; i++) {
                    normalized[i] *= sumScale;
                }
            }
        } else {
            // Normal case: scale to achieve sum = sqrt(2)
