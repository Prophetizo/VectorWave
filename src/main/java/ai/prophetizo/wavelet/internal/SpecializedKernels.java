...
public final class SpecializedKernels {
    ...
    public static void db4ForwardOptimized(double[] signal, double[] approx, double[] detail, int length) {
        ...
        for (; i < halfLength - VECTOR_LENGTH + 1; i += VECTOR_LENGTH) {
            ...
            for (int v = 0; v < VECTOR_LENGTH; v++) {
                int base = 2 * (i + v);
                double s0 = signal[(base + 0) % length];
        ...
        for (; i < halfLength; i++) {
            ...
            sum_a += DB4_H0 * signal[(base + 0) % length];
            ...
            sum_d += DB4_H7 * signal[(base + 0) % length];
    ...
    public static void sym4ForwardOptimized(double[] signal, double[] approx, double[] detail, int length) {
        ...
        for (int i = 0; i < halfLength; i++) {
            ...
            // Approximation
            double sum_a = SYM4_H0 * signal[(base + 0) % length];
            ...
            // Detail (QMF)
            double sum_d = SYM4_H7 * signal[(base + 0) % length];
